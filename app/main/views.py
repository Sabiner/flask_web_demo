...
def form_validate():
    ...
    if current_user.can(Permission.WRITE_ARTICLES) and form.validate_on_submit():
        ...
        db.session.add(post)
        return redirect(url_for('.form_validate'))
    ...
    return render_template('form_validate.html', form=form, posts=posts,
                           show_followed=show_followed, pagination=pagination)


@main.route('/user/<username>')
def user(username):
    print "这是来自GitHub的测试第二次"
...
def unfollow(username):
    ...
    if not current_user.is_following(user):
        flash('You are not follow %s yet.' % username)
        return redirect(url_for('.user', username=username))
...
@login_required
def show_all():
    resp = make_response(redirect(url_for('.form_validate')))
